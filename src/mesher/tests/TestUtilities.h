...
protected:
    ...
    Surfel s1{"id1", std::vector<FrameData>{
    ...
    }, std::vector<std::shared_ptr<Surfel>>{}, Eigen::Vector3f{1.0, 0.0, 0.0}};

    Surfel s1_neighbour{"id2", std::vector<FrameData>{
            FrameData{test_pixel_up, 10.0f, Eigen::Matrix3f::Identity(), Eigen::Vector3f::Zero()}

    }, std::vector<std::shared_ptr<Surfel>>{}, Eigen::Vector3f{1.0, 0.0, 0.0}};
    Surfel s1_not_neighbour{"id3", std::vector<FrameData>{
